...
namespace Taxi.Controls.Base.Controls
{
    public class TabControl : DevExpress.XtraTab.XtraTabControl, IListKeyPress
    {
        ...
        public List<IShKeyPress> GetList()
        {
            return this.TabPages.Where(p=>p.PageVisible).Select(p => (IShKeyPress)p).Where(p => p != null).ToList();
        }
    }
    ...
    public class TabPage : DevExpress.XtraTab.XtraTabPage, ILanguage, IShKeyPress
    {
