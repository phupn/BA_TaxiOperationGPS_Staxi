...
namespace Taxi.Controls.Base.Inputs
{
    ...
    /// </summary>
    [ToolboxItem(true)]
    public class InputLookUp : LookUpEdit, IShInput, IShKeyPress, ITextChange
    {
    ...
    public class InputLookUp<T> : InputLookUp, IShControl where T : ModelBase, new()
    {
    ...
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public class InputEnumLookUp<T> : InputLookUp, IShControl
    {
        /// <summary>
        /// Thêm chọn tất cả
        /// </summary>
        ...
        /// Sự kiện lựa chọn
        /// </summary>
        public event Action<EnumItemAttribute> SelectItem;
        ...
        protected override void OnEditValueChanged()
        {
            ...
            // Thực hiện sự kiện
            if (SelectItem != null) SelectItem(this.GetSelectedDataRow().As<EnumItemAttribute>());
        }
        ...
        protected virtual List<EnumItemAttribute> Data
        {
            get
            {
                var data = new EnumHandler<T>().GetEnumItem();
                if (IsAddAll)
                {
                    if (data.Any(p => p.Name == "Tất cả")) return data;
                    if (!data.Any(p=>(int)p.Value==0))
                    data.Insert(0, new Taxi.Common.EnumUtility.EnumItemAttribute("Tất cả") {Value = 0});
                    else 
                        data.Insert(0, new Taxi.Common.EnumUtility.EnumItemAttribute("Tất cả") { Value = -1 });
                }
                return data.OrderBy(T=>T.FieldName).ToList();
            }
        }
