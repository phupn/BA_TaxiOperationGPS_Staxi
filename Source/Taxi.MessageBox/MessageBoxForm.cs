...
namespace Taxi.MessageBox
{
    ...
    /// </summary>
    internal class MessageBoxForm : System.Windows.Forms.Form
    {
        #region Constants
        ...
        private MessageBoxButton _cancelButton = null;
        ...
        /// </summary>
        private MessageBoxIconBA _standardIcon = MessageBoxIconBA.None;
        ...
        public MessageBoxIconBA StandardIcon
        {
        ...
        public Icon CustomIcon
        {
            set
            {
                _standardIcon = MessageBoxIconBA.None;
                _iconImage = value;
            }
        }

        public MessageBoxButton CustomCancelButton
        {
        ...
        {
            if ((keyData==Keys.Escape|| (int)keyData == (int)(Keys.Alt | Keys.F4)))
            {
        ...
        private string GetLongestButtonText()
        {
            int maxLen = 0;
            string maxStr = null;
            foreach (MessageBoxButton button in _buttons)
            {
        ...
        private void AddOkButtonIfNoButtonsPresent()
        {
            if (_buttons.Count == 0)
            {
                MessageBoxButton okButton = new MessageBoxButton();
                okButton.Text = MessageBoxButtonsBA.OK.ToString();
                okButton.Value = MessageBoxButtonsBA.OK.ToString();
        ...
        /// <returns></returns>
        private Button GetButton(MessageBoxButton button, Size size, Point location)
        {
        ...
        /// <param name="location"></param>
        /// <returns></returns>
        private Button CreateButton(MessageBoxButton button, Size size, Point location)
        {
        ...
        private void DisableCloseIfMultipleButtonsAndNoCancelButton()
        {
            if (_buttons.Count > 1)
            {
                ...
                foreach (MessageBoxButton button in _buttons)
                {
                    if (button.Text == MessageBoxButtonsBA.Cancel.ToString() && button.Value == MessageBoxButtonsBA.Cancel.ToString())
                    {
            ...
            {
                _cancelButton = _buttons[0] as MessageBoxButton;
            }
            else
            {
        ...
        private void PlayAlert()
        {
            if (_playAlert)
            {
                if (_standardIcon != MessageBoxIconBA.None)
                {
        ...
        private void timerTimeout_Tick(object sender, EventArgs e)
        {
            ...
            switch (_timeoutResult)
            {
                ...
                case TimeoutResult.Timeout:
                    SetResultAndClose(MessageBoxResult.Timeout);
                    break;
            }
        }
        #endregion
