...
namespace Taxi.GUI 
{
    public partial class frmBC_GPS_1_BCCuocGoiDieuGPSTheoNgay : Form
    {
        private const int MAX_DATES = 100;
        DataTable g_dt = new DataTable();
        DataTable danhSachBinhQuan = new DataTable();
        public frmBC_GPS_1_BCCuocGoiDieuGPSTheoNgay()
        {
            InitializeComponent();
        }
        ...
        {
            danhSachBinhQuan = new TimKiem_BaoCao().GROUP_BC_GPS_1_BCCuocKhachGPSTheoNgay(TuNgay, DenNgay, txtMaNhanVien.Text.Trim());
        ...
        {   
               // Tao binh quan
            saveFileDialog1.FileName = FileTools.GetFilenameReport("BC_GPS_1_CuocKhachDieuHanhGPSTheoNgay", calTuNgay.Value, calDenNgay.Value, false);
