...
namespace Taxi.GUI
{
    public partial class frmConfirmXeDon : Form
    {
        ...
        private string G_IdCuocGoi = string.Empty;
        public int Result
        {
            get
            {
        ...
        public frmConfirmXeDon(KieuCanhBaoKhiNhapThongTin KieuCanhBao, string Message, long IdCuocGoi)
        {
            InitializeComponent();
            G_KieuCanhBao = KieuCanhBao;
            G_Message = Message;
            G_IdCuocGoi = IdCuocGoi.ToString();
        ...
        private void LoadFormByKieuCanhBao()
        {
            switch (G_KieuCanhBao)
            {
                ...
                case KieuCanhBaoKhiNhapThongTin.TrungXeDon:
                    ...
                    rbConfirmXeDon2.Text = "2.Không cho nhận";
                    break;
                case KieuCanhBaoKhiNhapThongTin.TrungXeNhanApp:
                    lblInfo.Text = G_Message;
                    rbConfirmXeDon1.Text = "1.Vẫn cho đón khách";
                    rbConfirmXeDon2.Text = "2.Không cho nhận";
                    break;
        ...
        private void txtXeDon_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                if (G_XeDonResult.Equals(txtXeDon.Text.Trim()))
                {
                    new MessageBox.MessageBoxBA().Show(string.Format("Phải nhập xe đón khác xe {0}", G_XeDonResult));
                    return;
                }
                else
                {
        ...
        private void frmConfirmXeDon_Load(object sender, EventArgs e)
        {

        }

        private void frmConfirmXeDon_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (G_KieuCanhBao == KieuCanhBaoKhiNhapThongTin.TrungXeNhanApp && DialogResult == System.Windows.Forms.DialogResult.OK)
            {
