...
namespace Taxi.GUI
{
    ...
    public partial class frmDieuHanhDienThoaiNEWCP_V4 : Form
    {
        #region ==========================Init=================================
        ...
        // --- form nhập dữ liệu ------------------
        private frmDienThoaiInputDataNew_V4 g_frmInput;
        ...
        public List<string> openedDialogs = new List<string>();

        frmDriversChat formDriverChat = new frmDriversChat();
        ...
        public frmDieuHanhDienThoaiNEWCP_V4()
        {
        ...
        public void ReloadControlBanCo()
        {
            try
            {
                ...
                splView.Panel1.Controls.Add(_controlDieuHanhBanCoBanCo);
            }
            catch (Exception ex)
            {
                new MessageBox.MessageBoxBA().Show(this, "Lỗi ReloadControlBanCo " + ex.Message,
        ...
        private void frmDieuHanhDienThoaiNEW_Load(object sender, EventArgs e)
        {
            try
            {
                if (DieuHanhTaxi.CheckConnection())
                {
                        ...
                        using (DataTable dt = QuanTriCauHinh.GetLines_LoaiXeOfPCDienThoai(g_IPAddress))
                        {
                            if (dt.Rows != null && dt.Rows.Count > 0)
                            {
                                ...
                                if (ThongTinCauHinh.GopKenh_TrangThai)
                                    G_LineGop = dt.Rows[0]["LineGop"] == DBNull.Value ? g_LinesDuocCapPhep : dt.Rows[0]["LineGop"].ToString();
                            }
                            else
                            {
                    ...
                    {
                        g_LinesDuocCapPhep = "1;2;3;4;5;6;7;8;9";
                    }
                    ...
                    if (g_LinesDuocCapPhep.Length > 0)
                    {
                        ...
                        CommonBL.WorkflowId = (int)Enum_Workflow.DienThoaiVien;
                        Config_LuoiDieu();
        ...
        /// <summary>
        /// Ẩn hiện control trên lưới dữ liệu
        /// </summary>
        /// <Modified>
        /// Name     Date         Comments
        ...
        private void AnHienControl()
        {
            ...
            {
                lblPhimDaDon.Visible = false;
                lblPhimTruot.Visible = false;
                lblPhimHuy.Visible = false;
                gridDienThoai.RootTable.Columns["LenhTongDai"].Visible = true;
                gridDienThoai.RootTable.Columns["GhiChuTongDai"].Visible = true;
                //gridDienThoai.RootTable.Columns["LenhDieuHanh"].Caption = "Lệnh ĐT";
            }
        ...
        private void LoadAllCuocGoiHienTai(string linesDuocCapPhep)
        {
            ...
            g_lstDienThoai = CuocGoi.DIENTHOAI_LayAllOfLineChoPhep_EnVangVip(linesDuocCapPhep);
        }
            ...
            DateTime thoiDiemNhanDulieuTruoc, ref bool hasCapNhat, ref bool hasThemMoi, ref DateTime DateMax)
        {
            ...
            List<CuocGoi> listCuocGoi = CuocGoi.DIENTHOAI_LayDSCuocGoiThayDoiByTongDai_V4_Khac(linesDuocCapPhep,
                thoiDiemNhanDulieuTruoc);
        ...
        private void TimVaCapNhatCuocGoi_Khac(ref List<CuocGoi> listCuocGoiHienTai, CuocGoi cuocGoi, ref bool hasThemMoi)
        {
            if (listCuocGoiHienTai != null && listCuocGoiHienTai.Count > 0)
            {
                ...
                if (index >= 0)
                {
                    ...
                    listCuocGoiHienTai[index].DaGuiDSXeNhan = cuocGoi.DaGuiDSXeNhan;
                    listCuocGoiHienTai[index].CenterConfirm = cuocGoi.CenterConfirm;
                }
                else
                {
        ...
        private void GetAllCuocGoiMoi(List<CuocGoi> listCuocGoiHienTai, string linesDuocCapPhep,
            DateTime thoiDiemNhanDulieuTruoc, ref bool hasCuocGoiMoi, ref DateTime DateMax)
        {
            hasCuocGoiMoi = false;
            List<CuocGoi> listCuocGoiMoi = CuocGoi.DIENTHOAI_LayDSCuocGoiMoi_EnVangVip(linesDuocCapPhep,
                thoiDiemNhanDulieuTruoc);
            if (listCuocGoiMoi != null && listCuocGoiMoi.Count > 0)
            {
                foreach (CuocGoi objCG in listCuocGoiMoi)
        ...
        /// <summary>
        /// Load các xe đang chat
        /// </summary>
        /// <Modified>
        /// Name     Date         Comments
        /// LuanBH  8/20/2015   created
        /// </Modified>
        private void LoadCacXeDangChat()
        {
            try
            {
                if (!formDriverChat.Visible)
                {
                    ...
                    {
                        formDriverChat = new frmDriversChat();
                        formDriverChat.SetProperties(g_LinesDuocCapPhep);
                        formDriverChat.SetVehiclesToGrid(cacXeDangChat);
                        formDriverChat.Show();
                    }
                }
            ...
            {
                LogError.WriteLogError("DieuHanhV4 : LoadCacXeDangChat", ex);
                return;
            }
        }
        ...
        private void HienThiAnhTrangThai_MauChu(GridEXRow row)
        {
            try
            {
                ...
                if (KiemTraCuocGoiCoLoiKhong(cuocGoi)) LenhLaiXeBackColor = Color.Red;
                ...
                if (cuocGoi.LenhTongDai == EnVangManagement.LENH_MOIKHACH)
                {
                    if (cuocGoi.LenhDienThoai == EnVangManagement.LENH_DAMOI || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_DAMOI
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_GAPXE || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_GAPXE
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_HOILAIDIACHI ||
                        cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_HOILAIDIACHI
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_KHONGXE || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_KHONGXE
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_MAYBAN || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_MAYBAN
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_KHONGLIENLACDUOC ||
                        cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_KHONGLIENLACDUOC
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_TRUOTCHUA || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_TRUOTCHUA
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_KHONGNOIGI || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_KHONGNOIGI
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_KOTRUCTIEP || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_KOTRUCTIEP
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_CHO5PHUT || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_CHO5PHUT
                        )
                    {
                        GridEXFormatStyle RowStyle = new GridEXFormatStyle();
                ...
                if (cuocGoi.LenhTongDai == EnVangManagement.LENH_6_KIENTRAKHACH
                    || cuocGoi.LenhTongDai == EnVangManagement.LENH_8_RADAUNGO)
                {
                    if (cuocGoi.LenhDienThoai == EnVangManagement.LENH_DAMOI || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_DAMOI
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_GAPXE || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_GAPXE
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_HOILAIDIACHI ||
                ...
                else if (cuocGoi.LenhDienThoai == EnVangManagement.LENH_HUYXE ||
                        cuocGoi.LenhDienThoai == EnVangManagement.LENH_G_GIUCXE ||
                    cuocGoi.LenhDienThoai.Contains(EnVangManagement.LENH_MATKN))
                {
                    GridEXFormatStyle RowStyle = new GridEXFormatStyle();
                    RowStyle.BackColor = Color.Tomato;
                    row.Cells["LenhDienThoai"].FormatStyle = RowStyle;
                }
                else if (cuocGoi.LenhDienThoai == EnVangManagement.LENH_KOTHAYXE || cuocGoi.LenhDienThoai == EnVangManagement.LENH_KTX_GoiChoKhach)
                {
                    GridEXFormatStyle RowStyle = new GridEXFormatStyle();
                    RowStyle.BackColor = Color.Yellow;
                    row.Cells["LenhDienThoai"].FormatStyle = RowStyle;
                }
                else if (cuocGoi.LenhTongDai == EnVangManagement.LENH_7_MOIKHACHLAN2)
                {
                    if (cuocGoi.LenhDienThoai == EnVangManagement.LENH_CHO5PHUT || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_CHO5PHUT
                        || cuocGoi.LenhDienThoai == EnVangManagement.LENH_DAMOI2 || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_DAMOI2
                        )
                    {
                        GridEXFormatStyle RowStyle = new GridEXFormatStyle();
                        RowStyle.BackColor = Color.White;
                ...
                else if (cuocGoi.LenhTongDai == EnVangManagement.LENH_9_TIEPTHIXEKHAC)
                {
                    if (cuocGoi.LenhDienThoai == EnVangManagement.LENH_DOIXE || cuocGoi.MOIKHACH_LenhMoiKhach == EnVangManagement.LENH_DOIXE)
                    {
                        GridEXFormatStyle RowStyle = new GridEXFormatStyle();
                        RowStyle.BackColor = Color.White;
                        row.Cells["LenhTongDai"].FormatStyle = RowStyle;
                ...
                else if (cuocGoi.LenhTongDai == EnVangManagement.LENH_HOILAIDIACHI)
                {
                    GridEXFormatStyle RowStyle = new GridEXFormatStyle();
                    RowStyle.BackColor = Color.Violet;
                    row.Cells["LenhTongDai"].FormatStyle = RowStyle;
                }
                else if (cuocGoi.LenhDienThoai == EnVangManagement.LENH_KHACHDAT)
                {
                    GridEXFormatStyle RowStyle = new GridEXFormatStyle();
                    RowStyle.BackColor = Color.Green;
                    row.Cells["LenhDienThoai"].FormatStyle = RowStyle;
                }
                if (cuocGoi.LenhTongDai == EnVangManagement.LENH_KHONGXE && cuocGoi.ThoiDiemGoi.AddMinutes(3) <= g_TimeServer)
                {
...
/*RowStyle.BackColor = Color.White;*/
        ...
        private void uiCommandBar2_CommandClick(object sender, Janus.Windows.UI.CommandBars.CommandEventArgs e)
        {
            ...
            else if (e.Command.Key == "cmdConfigCommand")
            {
                ...
                if (gridLayout != null)
                {
                    if (uiTabCuocGoiDangThucHien.SelectedTab.Name == "uiTabCuocGoiChoGiaiQuyet")
                    {
                        ComponentResourceManager resources =
                            new ComponentResourceManager(typeof (frmDieuHanhDienThoaiNEWCP_V4));
        ...
        private void frmDieuHanhDienThoaiNEWCP_V4_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        private void gridDienThoai_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (gridDienThoai.RowCount > 0)
            {
                ...
                if (gridDienThoai.SelectedItems != null && gridDienThoai.SelectedItems.Count > 0)
                {
                    ...
                    if (MangerCommand.ProcessMobileCommandByKey(e.KeyCode, ref cuocGoiRow, ref hasThucHienLenh)) 
                    { 
                    ...
                    else // 
                    {
                        ...
                        {
                            cuocGoiRow.LenhDienThoai = EnVangManagement.LENH_DANGGOI;
                        ...
                        else if (e.KeyCode == Keys.D1 || e.KeyCode == Keys.NumPad1)
                        {
                            ...
                            if (HasInitedTrip(cuocGoiRow))
                            {
                                EnVangProcess.SendACKInvite(cuocGoiRow, 1, "Đã mời");
                                cuocGoiRow.LenhDienThoai = EnVangManagement.LENH_DAMOI;
                                cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                                hasThucHienLenh = true;
                            }
                            else
                            {
                                ShowMessageDialog(msgDialog, EnVangManagement.LENH_DAMOI);
                            }
                        }
                        //================ 2 : máy bận
                        else if (e.KeyCode == Keys.D2 || e.KeyCode == Keys.NumPad2)
                        {
                            if (HasInitedTrip(cuocGoiRow))
                            {
                                EnVangProcess.SendACKInvite(cuocGoiRow, 0, "Máy bận");
                                cuocGoiRow.LenhDienThoai = EnVangManagement.LENH_MAYBAN;
                                cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                                hasThucHienLenh = true;
                            }
                            else
                            {
                                ShowMessageDialog(msgDialog, EnVangManagement.LENH_MAYBAN);
                            }
                        }
                        ...
                        else if (e.KeyCode == Keys.D3 || e.KeyCode == Keys.NumPad3)
                        {
                            if (HasInitedTrip(cuocGoiRow))
                            {
                                EnVangProcess.SendACKInvite(cuocGoiRow, 0, "Không nghe máy");
                                cuocGoiRow.LenhDienThoai = EnVangManagement.LENH_KHONGNGHEMAY;
                                cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                                hasThucHienLenh = true;
                            }
                            else
                            {
                                ShowMessageDialog(msgDialog, EnVangManagement.LENH_KHONGNGHEMAY);
                            }
                        }
                        //================ 4 : không nói gì
                        else if (e.KeyCode == Keys.D4 || e.KeyCode == Keys.NumPad4)
                        {
                            if (HasInitedTrip(cuocGoiRow))
                            {
                                EnVangProcess.SendACKInvite(cuocGoiRow, 0, "Không nói gì");
                                cuocGoiRow.LenhDienThoai = EnVangManagement.LENH_KHONGNOIGI;
                                cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                                hasThucHienLenh = true;
                            }
                            else
                            {
                                ShowMessageDialog(msgDialog, EnVangManagement.LENH_KHONGNOIGI);
                            }
                        }
                        //================ 5 : Đã mời lần 2
                        else if (e.KeyCode == Keys.D5 || e.KeyCode == Keys.NumPad5)
                        {
                            if (HasInitedTrip(cuocGoiRow))
                            {
                                EnVangProcess.SendACKInvite(cuocGoiRow, 1, "Đã mời lần 2");
                                cuocGoiRow.LenhDienThoai = EnVangManagement.LENH_DAMOI2;
                                cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                                hasThucHienLenh = true;
                            }
                            else
                            {
                                ShowMessageDialog(msgDialog, EnVangManagement.LENH_DAMOI2);
                            }
                        }
                        //================ 6 : Gửi tin nhắn/cho số
                        else if (e.KeyCode == Keys.D6 || e.KeyCode == Keys.NumPad6)
                        {
                            if (HasInitedTrip(cuocGoiRow))
                            {
                                EnVangProcess.SendACKGetPhone(cuocGoiRow, 1);
                                cuocGoiRow.LenhDienThoai = EnVangManagement.LENH_CHOSODT;
                                cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                                hasThucHienLenh = true;
                            }
                            else
                            {
                                ShowMessageDialog(msgDialog, EnVangManagement.LENH_CHOSODT);
                            }
                        }
                        //================ 7: không xe đã xin lỗi khách
                        else if (e.KeyCode == Keys.D7 || e.KeyCode == Keys.NumPad7)
                        {
                            ...
                            {
                                cuocGoiRow.LenhDienThoai = EnVangManagement.LENH_DAXINLOI;
                                cuocGoiRow.TrangThaiCuocGoi = TrangThaiCuocGoiTaxi.CuocGoiKhongXe;
                            ...
                            {
                                msgDialog.Show(this, string.Format("[Lệnh {0}] Cuội gọi phải là cuộc gọi taxi và chưa có xe nhận.", TrangThaiCuocGoiTaxi.CuocGoiKhongXe),
                                    "Thông báo",
                                    Taxi.MessageBox.MessageBoxButtonsBA.OK,
                                    Taxi.MessageBox.MessageBoxIconBA.Error);
                            }
                        ...
                        //================ H : Hủy xe / Hoãn
                        else if (e.KeyCode == Keys.H && Global.ChoPhepDienThoaiNhapXe)
                        {
                            string dialog = ShowMessageDialog(msgDialog, cuocGoiRow.DiaChiDonKhach, "Hủy cuốc {0}...?", Taxi.MessageBox.MessageBoxButtonsBA.YesNo, Taxi.MessageBox.MessageBoxIconBA.Question);
                            if (dialog == "Yes")
                            {
                                cuocGoiRow.LenhDienThoai = EnVangManagement.LENH_HUYXE;
                                cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                                hasThucHienLenh = true;
                                EnVangProcess.SendOperatorCancel(cuocGoiRow);
                        ...
                        //================ T : Truot
                        else if (e.KeyCode == Keys.T && Global.ChoPhepDienThoaiNhapXe)
                        {
                            string dialog = ShowMessageDialog(msgDialog, cuocGoiRow.DiaChiDonKhach, "Trượt cuốc {0}...?", Taxi.MessageBox.MessageBoxButtonsBA.YesNo, Taxi.MessageBox.MessageBoxIconBA.Question);
                            if (dialog == "Yes")
                            {
                                cuocGoiRow.LenhDienThoai = EnVangManagement.LENH_TRUOTCHUA;
                                cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.KetThuc;
                        ...
                        {
                            cuocGoiRow.LenhDienThoai = EnVangManagement.LENH_KOTHAYXE;
                            hasThucHienLenh = true;
                        }
                        //================ X: Đã đón
                        else if (e.KeyCode == Keys.X && Global.ChoPhepDienThoaiNhapXe)
                        {
                            if (HasInitedTrip(cuocGoiRow))
                            {
                            ...
                            {
                                ShowMessageDialog(msgDialog, EnVangManagement.LENH_DADON);
                            }
                        }
                        ...
                        #region Nhập xe
                        //================= / : chon nhap xe nhan
                        else if (e.KeyCode == Keys.Divide && Global.ChoPhepDienThoaiNhapXe)
                        {
                    ...
                    if (hasThucHienLenh)
                    {
                        CapNhatDongDuLieu(cuocGoiRow, msgDialog);
                    }
                }
            }
        }
        ...
        #region ----------- Hàm xử lý trên lưới cuộc gọi -----------
        /// <summary>
        /// Hiển thị message báo lỗi khi ấn phím tắt trên lưới
        /// </summary>
        /// <param name="msgDialog">The MSG dialog.</param>
        /// <param name="lenh">The lenh.</param>
        ...
        /// <Modified>
        /// Name     Date         Comments
        /// LuanBH  8/6/2015   created
        /// </Modified>
        private string ShowMessageDialog(
            MessageBox.MessageBoxBA msgDialog
            , string lenh
            , string textFormat = "[Lệnh {0}] Cuội gọi phải là cuộc gọi taxi và đã có xe nhận."
            , Taxi.MessageBox.MessageBoxButtonsBA buttons = Taxi.MessageBox.MessageBoxButtonsBA.OK
            , Taxi.MessageBox.MessageBoxIconBA icon = Taxi.MessageBox.MessageBoxIconBA.Error
            , string caption = "Thông báo")
        {
            return msgDialog.Show(this, String.Format(textFormat, lenh), caption, buttons, icon);
        }

        /// <summary>
        /// Kiểm tra đã init trip chưa
        /// </summary>
        ...
        /// LuanBH  8/6/2015   created
        /// </Modified>
        private bool HasInitedTrip(CuocGoi cuocGoi)
        {
            return cuocGoi.KieuCuocGoi == KieuCuocGoi.GoiTaxi
                && (!cuocGoi.KhongDungMobileService.Value && !string.IsNullOrEmpty(cuocGoi.XeDon) || (cuocGoi.KhongDungMobileService.Value && !string.IsNullOrEmpty(cuocGoi.XeNhan)));
            //return cuocGoi.KieuCuocGoi == KieuCuocGoi.GoiTax && cuocGoi.DaGuiDSXeNhan != null && cuocGoi.DaGuiDSXeNhan != 0 && cuocGoi.CenterConfirm != null && cuocGoi.CenterConfirm != 0;
        }
        /// <summary>
        ...
        public void CapNhatDongDuLieu(CuocGoi cuocGoiRow, MessageBox.MessageBoxBA msgDialog)
        {
            ...
            if (!updateSuccess)
            {
                if (msgDialog == null) msgDialog = new MessageBox.MessageBoxBA();
            ...
            else //
            {
                if (cuocGoiRow.TrangThaiLenh == TrangThaiLenhTaxi.KetThuc ||
                    cuocGoiRow.TrangThaiLenh == TrangThaiLenhTaxi.KetThucCuaDienThoai)
                {
                    if (cuocGoiRow.KieuCuocGoi == KieuCuocGoi.GoiKhac)
                    {
                        EnVangProcess.SendOperatorCancel(cuocGoiRow);
                        if (cuocGoiRow.FT_IsFT)
                        {
                    ...
                    {
                        EnVangProcess.SendConfirmDone(cuocGoiRow, 1);
                    }
        ...
        private bool NhapXeNhan(CuocGoi cuocGoiRow, int positionRowSelect, string value)
        {
            if (Global.ChoPhepDienThoaiNhapXe)
            {
                if (cuocGoiRow.KieuCuocGoi != KieuCuocGoi.GoiLai)
                {
                    ...
                    if (frmInput.ShowDialog() == DialogResult.OK)
                    {
                        var xeNhan = frmInput.GetGiaTriNhap();
                        if (EnVangManagement.ValidateNhieuSoHieuXe(xeNhan))
                        {
                            ShowMessageDialog(new MessageBox.MessageBoxBA(), EnVangManagement.MSG_17_XeNhanKhongDungDinhDang, "{0}");
                            return false;
                        }
                        else if (EnVangManagement.KiemTraPTDSThuocDSKhac(xeNhan, g_ListSoHieuXe))
                        {
                            ShowMessageDialog(new MessageBox.MessageBoxBA(), EnVangManagement.MSG_13_NHAPCHINHXACXE, "{0}");
                            return false;
                        }
                        cuocGoiRow.XeNhan = frmInput.GetGiaTriNhap();
                        cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.BoDam;
                        EnVangProcess.SendInitTrip(cuocGoiRow);
                        return true;
                    }
                }
                else
        ...
        {
            switch (cuocGoi.LenhDienThoai)
            {
                case EnVangManagement.LENH_DAMOI:
                    EnVangProcess.SendACKInvite(cuocGoi, 1, "Đã mời");
                    break;
                case EnVangManagement.LENH_MAYBAN:
                    EnVangProcess.SendACKInvite(cuocGoi, 0, "Máy bận");
                    break;
                case EnVangManagement.LENH_KHONGNGHEMAY:
                    EnVangProcess.SendACKInvite(cuocGoi, 0, "Không nghe máy");
                    break;
                case EnVangManagement.LENH_KHONGNOIGI:
                    EnVangProcess.SendACKInvite(cuocGoi, 0, "Không nói gì");
                    break;
                case EnVangManagement.LENH_DAMOI2:
                    EnVangProcess.SendACKInvite(cuocGoi, 1, "Đã mời 2");
                    break;
                case EnVangManagement.LENH_XINSODT:
                    EnVangProcess.SendACKGetPhone(cuocGoi, 1);
                    break;
                case EnVangManagement.LENH_HUYXE:
                    cuocGoi.LenhDienThoai = EnVangManagement.LENH_TRUOTCHUA;
                    cuocGoi.TrangThaiLenh = TrangThaiLenhTaxi.KetThuc;
                    cuocGoi.TrangThaiCuocGoi = TrangThaiCuocGoiTaxi.CuocGoiTruot;
                    break;
                case EnVangManagement.LENH_KOTHAYXE:
                    cuocGoi.LenhDienThoai = EnVangManagement.LENH_KOTHAYXE;
                    cuocGoi.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                    break;
                case EnVangManagement.LENH_DADON:
                    if (HasInitedTrip(cuocGoi))
                    {
                    ...
                    {
                        ShowMessageDialog(new MessageBox.MessageBoxBA(), EnVangManagement.LENH_DADON);
                    }
                    break;
                default:
                    break;
        ...
        /// <summary>
        /// Nhập dữ liệu cho cuộc gọi
        /// </summary>
        /// <param name="iRowPosition">The i row position.</param>
        /// <Modified>
        /// Name     Date         Comments
        /// LuanBH  8/10/2015   modified
        /// </Modified>
        private void NhapDuLieuVaoTruyenDi(int iRowPosition)
        {
            try
            {
                ...
                //End - Thu doi mau            
                g_frmInput = new frmDienThoaiInputDataNew_V4(cuocGoi, g_ListDataAutoComplete, g_CGLimit, G_DMVung_GPS)
                {
                ...
                if (g_frmInput.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    bool updateSuccess = false;
                    updateSuccess = CuocGoi.DIENTHOAI_UpdateThongTinCuocGoi_EnVangVip(cuocGoi);
                    if (!updateSuccess)
                    {
                        MessageBox.MessageBoxBA msgDialog = new Taxi.MessageBox.MessageBoxBA();
                    ...
                    {
                        if (g_frmInput.noteChanged)
                        {
                            EnVangProcess.SendText(cuocGoi);
                        }

                        if (g_frmInput.lenhDHChanged && !string.IsNullOrEmpty(cuocGoi.XeDon))
                        {
                        ...
                        if (!string.IsNullOrEmpty(cuocGoi.XeNhan) && (cuocGoi.DaGuiDSXeNhan == null || cuocGoi.DaGuiDSXeNhan == 0) && Global.ChoPhepDienThoaiNhapXe)
                        {
                        ...
                        else if (cuocGoi.KieuCuocGoi == KieuCuocGoi.GoiKhieuNai)
                        {
                            if (cuocGoi.Vung == 11)
                            {
                                ...
                                {
                                    MessageBox.MessageBoxBA msgDialog = new Taxi.MessageBox.MessageBoxBA();
                                    msgDialog.Show(this, "Không chuyển được dữ liệu sang bộ đàm, xin hãy liên hệ với quản trị", "Thông báo", Taxi.MessageBox.MessageBoxButtonsBA.OK, Taxi.MessageBox.MessageBoxIconBA.Error);
                                    //tra ve mau cu
                        ...
                        {
                            g_lstDienThoai.Remove(cuocGoi);
                            EnVangProcess.SendOperatorCatched(cuocGoi);
                            HienThiTrenLuoi(false, false);
                            //Cap nhat vao Mem
                            if (bwSync_RemoveCuocGoi != null)
                        ...
                        if (cuocGoi.SoLuong > 1)
                        {
                            CuocGoi.EnVangVIP_TaoNhieuCuocGoi(cuocGoi.IDCuocGoi, cuocGoi.SoLuong);
                        }
                    }
                }
                // Thi?t l?p l?i form
        ...
        public void TimerCapturePhone_Tick(object sender, EventArgs eArgs)
        {
            try
            {
                ...
                if (G_LineGop != "" && g_TimeServer.TimeOfDay >= ThongTinCauHinh.GopKenh_GioBD && g_TimeServer.TimeOfDay <= ThongTinCauHinh.GopKenh_GioBD)
                {
                ...
                GetAllCuocGoiMoi(g_lstDienThoai, g_LinesDuocCapPhep, g_ThoiDiemNhanDulieuTruoc, ref hasCuocGoiMoi, ref DateMax);
                LoadCacXeDangChat();
                //XuLyNhanLenhTuLaiXe();
                if (hasCuocGoiMoi)
                {
                    ...
                    g_ThoiDiemNhanDulieuTruoc = g_ThoiDiemNhanDulieuTruoc.AddSeconds(1);
        ...
        private void XuLyThongTinThayDoi(CuocGoi cuocGoiThayDoi)
        {
            ...
            if (g_frmInput != null && g_frmInput.GetCuocGoi.IDCuocGoi == cuocGoiThayDoi.IDCuocGoi)
            {
                g_frmInput.XuLyThongTinXeDon(cuocGoiThayDoi);
                g_frmInput.XuLyThongTinLenhLaiXe(cuocGoiThayDoi);
            }
        }
        ...
        {
            List<MessageConfirm> messages = CuocGoi.DIENTHOAI_LayMessageConfirm_EnVangVIP(g_LinesDuocCapPhep, true, string.Empty);
            bool choPhepDienThoaiNhapXe = Global.ChoPhepDienThoaiNhapXe;


            foreach (var message in messages)
            {
                // Kiểm tra có cần mở dialog và đã mở hay chưa
                if (message.CanConfirm && EnVangManagement.ChoPhepHienMessageConfirm(message.MaMessage))
                {
                    if (openedDialogs.IndexOf(message.MaMessage + "_" + message.XeDon) == -1)
                    {
                        // Thêm tên của dialog sẽ mở vào mảng  để kiểm tra mở trùng
                        openedDialogs.Add(message.MaMessage + "_" + message.XeDon);
                        EnVangManagement.OpenFormInfo(message, this);
                    }
                }
                else
                {
        ...
        /// LuanBH  8/7/2015   created
        /// </Modified>
        public void XuLyMessageKhongCanConfirm(MessageConfirm message)
        {
            if (message.MaMessage == EnVangManagement.MA_LENH_DAKETTHUC)
            {
                foreach (var cuocGoi in g_lstDienThoai)
                {
                    if (cuocGoi.IDCuocGoi == message.IDCuocGoi)
                    {
                        cuocGoi.TrangThaiCuocGoi = TrangThaiCuocGoiTaxi.CuocGoiDonDuocKhach;
                        cuocGoi.TrangThaiLenh = TrangThaiLenhTaxi.KetThucCuaDienThoai;
                        CuocGoi.DIENTHOAI_SuaMessageConfirm_EnVangVip(cuocGoi.IDCuocGoi
                            , EnVangManagement.MessageCodes[EnVangManagement.MA_LENH_DAKETTHUC]
                            , EnVangManagement.MA_LENH_DAKETTHUC, false, message.SoHieuXe);
            ...
            DateTime thoiDiemNhanDulieuTruoc, ref bool hasCuocGoiThayDoi, ref DateTime DateMax)
        {
            hasCuocGoiThayDoi = false;
            // Lấy ds cuộc gọi có thay đổi thông tin của tổng đài
            List<CuocGoi> listCuocGoi = CuocGoi.DIENTHOAI_LayDSCuocGoiThayDoiByTongDai_V4(linesDuocCapPhep,
                thoiDiemNhanDulieuTruoc);
            if (listCuocGoi != null && listCuocGoi.Count > 0)
            {
                foreach (CuocGoi objCG in listCuocGoi)
        ...
        /// LuanBH  8/10/2015   created
        /// </Modified>
        private void ThongBaoCoXeDon(CuocGoi cuocGoi)
        {
            var message = new MessageConfirm();
            message.XeDon = cuocGoi.XeDon;
            message.DiaChiDonKhach = cuocGoi.DiaChiDonKhach;
            message.MaMessage = EnVangManagement.MA_LENH_DADON;
            EnVangManagement.OpenFormInfo(message, this);
        }
            ...
            bool IsCapNhatCuaDienThoai)
        {
            if (listCuocGoiHienTai != null && listCuocGoiHienTai.Count > 0)
            {
                ...
                if (index >= 0)
                {
                    //if (string.IsNullOrEmpty(listCuocGoiHienTai[index].XeDon) && !string.IsNullOrEmpty(cuocGoi.XeDon))
                    //{
                    //    ThongBaoCoXeDon(cuocGoi);
                    //}
                    ...
                    if (!IsCapNhatCuaDienThoai) // cap nhat theo du lieu tong dai gui sang
                    {
                        ...
                        listCuocGoiHienTai[index].MH_LenhLaiXe = cuocGoi.MH_LenhLaiXe;
                        listCuocGoiHienTai[index].MH_SoLanGuiToiXe = cuocGoi.MH_SoLanGuiToiXe;
                        listCuocGoiHienTai[index].MH_ThoiDiemGuiXe = cuocGoi.MH_ThoiDiemGuiXe;
                        listCuocGoiHienTai[index].GhiChuLaiXe = cuocGoi.GhiChuLaiXe;
                        listCuocGoiHienTai[index].KhongDungMobileService = cuocGoi.KhongDungMobileService;
                    }
                    else
                    {
                        if (cuocGoi.TrangThaiLenh == TrangThaiLenhTaxi.KetThucCuaDienThoai)
                ...
                XuLyThongTinThayDoi(listCuocGoiHienTai[index]);
