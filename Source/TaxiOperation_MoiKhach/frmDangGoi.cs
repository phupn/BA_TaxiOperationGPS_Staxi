...
namespace Taxi.GUI
{
    public partial class frmDangGoi : Form
    {
        ...
        {
            //this.lblSoGoi.Text += phoneNumber + " -- " + lineIPPBX; 
            if (Config_Common.Asterisk_QuickCall)
            {
                ...
                else if (!string.IsNullOrEmpty(Config_Common.Zlink_LinkDial))
                {
                    QuaySoGoiDen_Zlink(lineIPPBX, phoneNumber);
                }
                else
                {
                    QuaySoGoiDien();
        ...
        public bool QuaySoGoiDen_Zlink(string line, string phoneNumber)
        {
        http://192.168.1.198/zlinkclick2dial.php?customernumber=0972284850&agentnumber=117&crmid=123456
            string url = string.Format("{0}{1}&agentnumber={2}&crmid=123456",
                    Config_Common.Zlink_LinkDial,
                    phoneNumber,
                    line,
                    ThongTinDangNhap.USER_ID
                    );
            if (Global.IsDebug)
                new MessageBox.MessageBoxBA().Show(url);
            string result = ProcessGoiDi(url);
            Text = string.Format("Đang gọi đi : {0} thành công", phoneNumber);
            if (Global.IsDebug)
                new MessageBox.MessageBoxBA().Show(result);
            return result.Contains("CSKH");
        }
