    ...
    public partial class frmDienThoaiInputDataNew_V3 : Form
    {
        ...
        private void frmDienThoaiInputDataMaiLinh_Load(object sender, EventArgs e)
        {
           ...
           // if (!g_CuocGoi.FT_IsFT)
            ...
            if (g_CuocGoi.G5_Type == Enum_G5_Type.DieuApp && !g_CuocGoi.FT_IsFT)
            {
                ShowDieuApp(true);
            ...
            if (Config_Common.AnKieuCuocGoiKhac)
            {
                chkGoiKhac.Visible = false;
            }
        }
        ...
        private void SetDuLieuLenForm(CuocGoi cuocGoi)
        {
            if (cuocGoi != null)
            {
                 ...
                 * 
                 * */
                if (cuocGoi.Vung > 0 && (cuocGoi.BookId != Guid.Empty || cuocGoi.FT_IsFT || !string.IsNullOrEmpty(cuocGoi.LenhLaiXe)))
                {
                    cbkCuocDieuApp.Enabled = false;
                }
        ...
        private void HienThiControl_GoiKhieuNai()
        {
            if (chkGoiKhieuNai.Checked)
            {
                ...
                chkGoiKhac.Checked = false;
                pnlTongDai.Visible = false;
                new Taxi.Controls.KhieuNai_PhanAnh.frmGhiNhanPhanAnh(ThongTinDangNhap.RoleNhanVienPA, g_CuocGoi).ShowDialog();
            }
        ...
        {
            if ((cuocGoi.Vung == 0 || cuocGoi.Vung == null) || (cbkCuocDieuApp.Checked && cbkCuocDieuApp.Enabled))
            {
                cuocGoi.G5_Type = cbkCuocDieuApp.Checked ? Enum_G5_Type.DieuApp : Enum_G5_Type.ChuyenSangDam;
            }
        ...
        {            //
            BangMaValidate maValidate =Config_Common.AnKieuCuocGoiKhac&&IsKhachDat?BangMaValidate.ValidateSuccess: ValidateFormNhap(g_CuocGoi);
        ...
        protected override bool ProcessDialogKey(Keys keyData)
        {
            ...
            {
                if (chkGoiKhac.Enabled && chkGoiKhac.Visible)
                {
                    chkGoiKhac.Focus();
                    if (chkGoiKhac.Checked)
                    {
            ...
            else if (keyData == Keys.Enter) // Mo nhap du lieu dong 1
            {
                ...
                {
                    if (txtDiaChi_GPS.IsFocused == false)
                    {
            ...
            {
                using (frmTraCuuBangGiaGoc frmTraCuuBangGiaGoc = new frmTraCuuBangGiaGoc())
                {
                    frmTraCuuBangGiaGoc.ShowDialog();
                }
                return true;
            }
        ...
        private void ShowDieuApp(bool isDieuApp)
        {
            try
            {
                if (isDieuApp)
