...
namespace Taxi.GUI
{
    ...
    public partial class frmDienThoaiInputDataNew_V3 : Form
    {
        ...
        private const string MSG_17_NhapXeDonKhongThuocXeNhan = "Xe đón không thuộc xe nhận.";
        ...
        private KhachHangDungThe KHInfo;
        #endregion
        ...
        private void frmDienThoaiInputDataMaiLinh_Load(object sender, EventArgs e)
        {
            ...
            KHInfo = KhachHangDungThe.Inst.GetKH(g_CuocGoi.PhoneNumber, DateTime.Now.Date);
            if (KHInfo != null)
            {
        ...
        private void SetDuLieuLenForm(CuocGoi cuocGoi)
        {
            if (cuocGoi != null)
            {
                ...
                if (cuocGoi.IsHeThong == 1)
                    chkTuSon.Checked = true;
                else if (cuocGoi.IsHeThong == 2)
                    chkHaNoi.Checked = true;
        ...
        private BangMaValidate ValidateFormNhap(CuocGoi cuocGoi)
        {
            if (chkGoiTaxi.Checked && Global.MoHinh == MoHinh.TongDaiMini)
            {
                ...
                if (g_XeDon != null && g_XeDon.Length > 0)
                {
                    ...
                    //if (Config_Common.TongDai_HienThiCanhBao_XeDonTrungXeNhan > 0 && KiemTraXeDonThuocXeNhan(g_XeDon, g_XeNhan))
                    //{
                    //    return BangMaValidate.NhapXeDonKhongThuocXeNhan;
                    //}
                }
            }
            // Không nhập địa chỉ
        ...
        private void HienThiThongBao(BangMaValidate maValidate)
        {
            ...
            else if (maValidate == BangMaValidate.NhapXeDonKhongThuocXeNhan)
            {
                lblThongBao.Text = MSG_17_NhapXeDonKhongThuocXeNhan;
            }
        }
        ...
        private void GetThongTinNhapMoi(ref CuocGoi cuocGoi)
        {
            ...
            if (chkGoiTaxi.Checked)
            {
                ...
                if (Global.MoHinh == MoHinh.TongDaiMini)
                {
                    ...
                    if (chkHoan.Checked)
                    {
                        cuocGoi.LenhDienThoai = LENH_HUYXE;
                    ...
                    {
                        cuocGoi.LenhDienThoai = LENH_TRUOTCHUA;
                    ...
                    else if (chkKhongXe.Checked)
                    {
                        ...
                        cuocGoi.XeDenDiem = "";
                        cuocGoi.XeDon = "";
                        cuocGoi.LenhDienThoai = LENH_KHONGXE;
                    }
                }
            }
        ...
        protected override bool ProcessDialogKey(Keys keyData)
        {
            ...
            {
                new frmGhiNhanPhanAnh("" , g_CuocGoi).Show();
                return true;
            }
        ...
        private void MainMap_MouseMove(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right && isMouseDown)
            {
                if (MainMap.OverlayCustom != null && MainMap.OverlayCustom.Markers.Count > 0 && MainMap.MarkerCustomer.IsVisible)
                {
                    ...
                    string strDiaChi = Service_Common.GetAddressByGeoBA((float)MainMap.MarkerCustomer.Position.Lat, (float)MainMap.MarkerCustomer.Position.Lng);
        ...
        private void setMarker()
        {
            if (top.Markers.Count == 0)
            {
                ...
                else
                {
                    ...
                    string toaDo = Service_Common.GetGeobyAddress(strDiaChi, ThongTinCauHinh.GPS_TenTinh);
        ...
        private void chkHaNoi_CheckedChanged(object sender, EventArgs e)
        {
            if (chkHaNoi.Checked)
            {
                chkTuSon.Checked = false;
            }
        }

        private void chkTuSon_CheckedChanged(object sender, EventArgs e)
        {
            if (chkTuSon.Checked)
            {
                chkHaNoi.Checked = false;
            }
