...
namespace Taxi.GUI
{
    public partial class frmDieuHanhDienThoaiNEWCP_V3 : Form
    {
        ...
        #region ------- LENH -------
        private string LENH_DAMOI = "Đã mời";
        ...
        private string LENH_7 = "Ko nói gì";
        ...
        private       string LENH_9 = "Giữ Rồi";
        ...
        private const string LEND_KOLIENLACDUOC = "Ko liên lạc đc";
        ...
        #region  ------- Define  -------
        //-----------------------------------------------------------------------------------------
        private List<CuocGoi> g_lstDienThoai;
        ...
        private int g_TimerStep_5 = 0; //
        private int g_TimerCheckLimitCG = 0; //5 phut check so cuoc goi taxiOperation
        private bool g_CGLimit = false; //
        /// <summary>
        /// Line mặc định trong cấu hình
        /// </summary>
        private string g_LinesDuocCapPhep = string.Empty;
        /// <summary>
        /// Line nhân viên tự cấu hình (có nhận cuốc gọi)
        /// </summary>
        private string g_LinesDuocCapPhep_Custom = string.Empty;
        /// <summary>
        /// Line nhân viên tự cấu hình (không nhận cuốc gọi)
        /// </summary>
        private string g_LinesDuocCapPhep_Support = string.Empty;
        ...
        private bool G_IsChangingLine = false;
        #endregion
        ...
        private void frmDieuHanhDienThoaiNEW_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {                  
                    ...
                    lblXeDenDiem_Help.Visible = Global.MoHinh == MoHinh.TongDaiMini;
                  ...
                  //  lblSpace
                    ...
                    if (Debugger.IsAttached)
                    {
                        g_LinesDuocCapPhep = "114;116";//"101;102;103;104;105;106;107;108;109;101";
                    }

                    if (g_LinesDuocCapPhep.Length > 0)
                    {
                        g_LinesDuocCapPhep_Custom = g_LinesDuocCapPhep_Support = g_LinesDuocCapPhep;
                        SetControl_LoaiXe_MacDinh();
                       ...
                       // CommandManager.function += CommandManager_Function;
                        #region config Taxi Phương Trang
                        if (license.idxCompany == 47)
                        {
                            lblXeNhan_Help.Text = "Phím [/] : Xe đăng ký";
                            //lblXeDenDiem_Help.Text = "Xe nhận";
                            lblGiucXe_Help.Text = "Phím G : Khách hối";
                        }
                        #endregion
                        ...
                        {
                            lblPhim5.Text = "Phím 5:Thuê bao";
                            LENH_5 = "Thuê bao";

                            lblPhim7.Text = "Phím 7:Từ chối";
                            LENH_7 = "Từ chối nghe máy";

                            lblPhim9.Text = "Phím 9:Thấy xe";
                            LENH_9 = "Thấy xe";

                            lblPhimC.Text = "Phím C:Chờ khách";
                            LENH_CHOKHACH = "Chờ khách";
                        ...
                        {
                            LENH_9 = "Gọi Hộ";
                            lblPhim9.Text = "Phím 9 : Gọi hộ";
                        }
                        if (!CheckIn()) return;

                        #region TongDai Siemens 
                        ...
                        Action ProcessVung = () =>
                        {
                            ...
                            if (G_DMVung_GPS != null)
                            {
                                ...
                                {
                                    if(!G_VungToaDo.ContainsKey(item.KenhVung))
                        ...
                        g_frmInput = new frmDienThoaiInputDataNew_V3(g_ListDataAutoComplete, g_CGLimit, G_DMVung_GPS);
        ...
        bool CommandManager_Function(CuocGoi cuocGoi, TaxiFunction function)
        {
            switch (function)
            {
                case TaxiFunction.NhapXeNhan:
                    return false;
                case TaxiFunction.NhapXeDenDiem:
                    return false;
                case TaxiFunction.NhapXeDungDiem:
                    return false;
                case  TaxiFunction.NhapXeDon:
                    return false;
                case TaxiFunction.NhapGhiChu:
                    return false;
                case TaxiFunction.MoBanDo:
                    return false;
            }
            return false;
        }
        ...
        /// phupn      8/01            Create
        /// </Modified>
        private void GetAllCuocGoiMoi_Khac(List<CuocGoi> listCuocGoiHienTai, string linesDuocCapPhep, DateTime thoiDiemNhanDulieuTruoc, ref bool hasCuocGoiMoi)
        {
            hasCuocGoiMoi = false;
        ...
        private void CapNhatCuocGoiKetThuc_Khac(ref List<CuocGoi> listCuocGoiHienTai, string linesDuocCapPhep, ref bool hasCapNhat)
        {
            hasCapNhat = false;
        ...
        private void HienThiTrenLuoi_Khac(bool hasCuocGoiMoi, bool hasThayDoiThongTin)
        {
            try
            {
                if (hasThayDoiThongTin)
                {
                    if (hasCuocGoiMoi)
                    {
                        gridDienThoai_All.DataSource = g_lstDienThoai_Khac;
        ...
        private void GetAllCuocGoiMoiV2(List<CuocGoi> listCuocGoiHienTai, string linesDuocCapPhep,
           DateTime thoiDiemNhanDulieuTruoc, ref bool hasCuocGoiMoi, ref int countStaxi, ref DateTime DateMax)
        {
            hasCuocGoiMoi = false;
            List<CuocGoi> listCuocGoiMoi;
            if (ThongTinCauHinh.FT_Active)
                listCuocGoiMoi = CuocGoi.DIENTHOAI_LayDSCuocGoiMoi_FT(linesDuocCapPhep, thoiDiemNhanDulieuTruoc);
            else
                listCuocGoiMoi = CuocGoi.DIENTHOAI_LayDSCuocGoiMoi_FTNotFT(linesDuocCapPhep, thoiDiemNhanDulieuTruoc);
        ...
        private void HienThiTrenLuoi(bool hasCuocGoiMoi, bool hasThayDoiThongTin)
        {
            try
            {
                if (hasThayDoiThongTin)
                {
                    ...
                    if (hasCuocGoiMoi)
                    {
                        //gridDienThoai.DataSource = g_lstDienThoai;
            ...
            try
            {
                ...
                if (cuocGoi.LenhTongDai == LENH_MOIKHACH)
                {
                    if (cuocGoi.LenhDienThoai == LENH_DAMOI || cuocGoi.MOIKHACH_LenhMoiKhach == LENH_DAMOI
                        ...
                        || cuocGoi.LenhDienThoai == LENH_5 ||
                        cuocGoi.MOIKHACH_LenhMoiKhach == LENH_5
                        || cuocGoi.LenhDienThoai == LENH_TRUOTCHUA || cuocGoi.MOIKHACH_LenhMoiKhach == LENH_TRUOTCHUA
                        || cuocGoi.LenhDienThoai == LENH_7 || cuocGoi.MOIKHACH_LenhMoiKhach == LENH_7
                        || cuocGoi.LenhDienThoai == LENH_9 || cuocGoi.MOIKHACH_LenhMoiKhach == LENH_9
                ...
                if (cuocGoi.FT_IsFT)
                {
                    row.Cells["ImageCol"].ImageIndex = 14;
                }
                else
                {
...
/*RowStyle.BackColor = Color.White;*/
                ...
                if (cuocGoi.FT_IsFT)
                {
                    ...
                    if (cuocGoi.KieuCuocGoi == KieuCuocGoi.GoiLai || (cuocGoi.Vung == 0 && cuocGoi.SoLanGoi >= 2) || cuocGoi.LenhDienThoai == LENH_G_GIUCXE)
                    {
                        GridEXFormatStyle RowStyle = new GridEXFormatStyle() {ForeColor = Color.Red};
                        RowStyle.FontSize = Config_Common.LuoiCuocGoi_FontSize_TongDai;
                        row.Cells["DiaChiDonKhach"].FormatStyle = RowStyle;
        ...
        private void uiCommandBar2_CommandClick(object sender, Janus.Windows.UI.CommandBars.CommandEventArgs e)
        {
            ...
            {
                using (frmChenMoiMotCuocDienThoai frm = new frmChenMoiMotCuocDienThoai(g_LinesDuocCapPhep,g_ListDataAutoComplete))
                {
                    frm.ShowDialog();
                }
            }
            ...
            {
                using (frmChenMoiMotCuocDienThoai frmChenMoiMotCuocDienThoai =new frmChenMoiMotCuocDienThoai(g_LinesDuocCapPhep, g_ListDataAutoComplete))
                {
            ...
            #endregion
            else if (e.Command.Key == "cmdPAKN_Input")
            {
                new frmGhiNhanPhanAnh("").Show();
            }
            else if (e.Command.Key == "cmdTraCuuLoiViPham")
            {
                new FrmTraCuLoiViPham().Show();
            }
        }
        ...
        private void uiCommandBar1_CommandClick(object sender, Janus.Windows.UI.CommandBars.CommandEventArgs e)
        {
            ...
            {
                using (frmChenMoiMotCuocDienThoai frmChenMoiMotCuocDienThoai =
        ...
        private void Search()
        {
            ...
            gridEX_All.DataSource = lstCuocGoi;
        ...
        private void frmDieuHanhDienThoaiNEWCP_V3_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            try
            {
                ...
                //if (bwSync_UpdateCuocGoi != null) bwSync_UpdateCuocGoi.Dispose();
                this.Dispose(true);
        ...
        private void gridDienThoaiAll_KeyDown(object sender, KeyEventArgs e)
        {
            MessageBox.MessageBoxBA msgDialog = new MessageBox.MessageBoxBA();
            if (gridDienThoai_All.RowCount > 0)
            {
                if (gridDienThoai_All.SelectedItems.Count <= 0)
                {
                    if (e.KeyCode == Keys.Up)
                    {
                        gridDienThoai_All.Row = 0;
                    }
                    else
                    {
                        gridDienThoai_All.Row = gridDienThoai_All.RowCount - 1;
                    }
                }
                ...
                if (gridDienThoai_All.SelectedItems != null && gridDienThoai_All.SelectedItems.Count > 0)
                {
                    ...
                    else if (e.KeyCode == Keys.D3 || e.KeyCode == Keys.NumPad3)
                    {
                        if (cuocGoiRow.KieuCuocGoi == KieuCuocGoi.GoiTaxi &&
                            (cuocGoiRow.TrangThaiCuocGoi == TrangThaiCuocGoiTaxi.CuocGoiKhongXe ||
                                ...
                                cuocGoiRow.LenhTongDai == "Ko xe xl khách") &&
                    ...
                    else if (e.KeyCode == Keys.D5 || e.KeyCode == Keys.NumPad5)
                    {
                        if (cuocGoiRow.KieuCuocGoi == KieuCuocGoi.GoiTaxi)
                        {
                            cuocGoiRow.LenhDienThoai = LENH_5;
                            cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                            hasThucHienLenh = true;
                        }
                        else
                        {
                            msgDialog.Show(this,
                                String.Format("[Lệnh {0}] Cuội gọi phải là cuộc gọi taxi.", LENH_5),
                    ...
                    else if (e.KeyCode == Keys.D7 || e.KeyCode == Keys.NumPad7)
                    {
                        if (cuocGoiRow.KieuCuocGoi == KieuCuocGoi.GoiTaxi)
                        {
                            cuocGoiRow.LenhDienThoai = LENH_7;
                            cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                            hasThucHienLenh = true;
                        }
                        else
                        {
                            msgDialog.Show(this,
                                String.Format("[Lệnh {0}] Cuội gọi phải là cuộc gọi taxi.", LENH_7),
                                "Thông báo", MessageBox.MessageBoxButtonsBA.OK, MessageBox.MessageBoxIconBA.Error);
                        }
                    }
                    //================ 8 : Hủy xe / Hoãn
                    ...
                    {
                        cuocGoiRow.LenhDienThoai = LENH_9;
                        cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                        hasThucHienLenh = true;
                    }
                    ...
                    else if (e.KeyCode == Keys.Delete)
                    {
                        ...
                        string dialog = msgDialog.Show(
                            ...
                            MessageBox.MessageBoxButtonsBA.YesNo, MessageBox.MessageBoxIconBA.Question);
                        if (dialog == "Yes")
                        {
                            hasThucHienLenh = true;
                    ...
                    //================ K :Thấy xe// Không trực tiếp
                    else if (e.KeyCode == Keys.K)
                    {
                        cuocGoiRow.LenhDienThoai = LENH_THAYXE;//LENH_KOTRUCTIEP;
                        cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                        hasThucHienLenh = true;
                    }
                    //================ K : Thuê bao//Không liên lạc được.
                    else if (e.KeyCode == Keys.L)
                    {
                        cuocGoiRow.LenhDienThoai = LENH_THUEBAO;// LEND_KOLIENLACDUOC;
                        cuocGoiRow.TrangThaiLenh = TrangThaiLenhTaxi.DienThoai;
                        hasThucHienLenh = true;
                    }
                    //================ space: đang gọi
                    ...
                    else if (e.KeyData == (Keys.Control | Keys.C))
                    {
                        try
                        {
                            GridEXColumn col = gridDienThoai_All.CurrentColumn;
            ...
            try
            {
                ...
                if (g_frmInput.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    if (Global.MoHinh == MoHinh.TongDaiMini)
                        ...
                        updateSuccess = CuocGoi.FT_DIENTHOAI_UpdateThongTinCuocGoiV2(cuocGoi);
        ...
        private void InitTimerCapturePhone()
        {
            ...
            TimerDateTime.Interval = TimerLength;
            ...
            try
            {
                ...
                if (G_LineGop != "" 
                    && (Config_Common.GopLine || 
                        (ThongTinCauHinh.GopKenh_TrangThai 
                        && g_TimeServer.TimeOfDay >= ThongTinCauHinh.GopKenh_GioBD 
                        && g_TimeServer.TimeOfDay <= ThongTinCauHinh.GopKenh_GioBD)
                    ))
                {
                    g_LinesDuocCapPhep = g_LinesDuocCapPhep_Custom = g_LinesDuocCapPhep_Support = G_LineGop;
                }
                GetAllCuocGoiMoiV2(g_lstDienThoai, g_LinesDuocCapPhep_Custom, g_ThoiDiemNhanDulieuTruoc, ref hasCuocGoiMoi, ref countCuocStaxi, ref DateMax);
                if (hasCuocGoiMoi)
                {
                ...
                if (Global.GridConfig_CuocGoi == Grid_Config.Has_Grid_Right)
                {
                    ...
                    if (g_TimerStep_Khac_Update >= 3)
                    {
                        ...
                        CapNhapCuocGoiTuTongDai_Khac(ref g_lstDienThoai_Khac, g_LinesDuocCapPhep, g_ThoiDiemNhanDuLieuTruocTongDai_Khac, ref hasCuocGoiThayDoi, ref hasCuocGoiMoi, ref DateMax);
                ...
                //if (g_MoHinh != MoHinh.TongDaiMini)
                {
                    ...
                    if (g_TimerStep >= 3) // 3 giây cập nhật một lần
                    {
                        ...
                        hasCuocGoiThayDoi = false;
                        DateMax = DateTime.MinValue;
                        CapNhapCuocGoiTuTongDai(ref g_lstDienThoai, g_LinesDuocCapPhep_Support, g_ThoiDiemNhanDuLieuTruocTongDai, ref hasCuocGoiThayDoi, ref DateMax, ref hasCuocGoiMoi);
                        if (hasCuocGoiThayDoi)
                        {
                        ...
                        if (Global.GridConfig_CuocGoi == Grid_Config.Has_Grid_Right)
                        {
                            CapNhatCuocGoiKetThuc_Khac(ref g_lstDienThoai_Khac, g_LinesDuocCapPhep_Support, ref hasCuocGoiMoi);
                            if (hasCuocGoiMoi)
                            {
                ...
                g_TimerStep_5++;
                if (g_TimerStep_5 >= 3) // 3 giây cập nhật một lần
                {
                    hasCuocGoiMoi = false;
                    CapNhatCuocGoiKetThucTuTongDai(ref g_lstDienThoai, g_LinesDuocCapPhep_Support, ref hasCuocGoiMoi);
                    if (hasCuocGoiMoi)
                    {
                        HienThiTrenLuoi(true, true);
                    }
                ...
                if ((g_boolChuyenTabCuocGoiKetThuc))
                {
                    try
                    {
                        LoadCacCuocGoiKetThuc(g_LinesDuocCapPhep_Support, g_SoDong);
                    }
                    catch (Exception ex)
                    {
                        LogError.WriteLogError("LoadCacCuocGoiKetThuc", ex);
        ...
        private void getXeDeCuByToaDo(double kinhDo, double viDo, string loaiXe, DateTime ThoiDiemGoi, long IDCuocGoi, string PhoneNumber, string DiaChiDonKhach, ref string DsXeDeCu, ref string DsXeDeCu_TD)
        {
            try
            {
                string loaiXeGPS = "";
                ...
                int SoXeTraVe = 4 + 1;
                // xử lý loại xe
                if (string.IsNullOrEmpty(loaiXe) || loaiXe.Trim() == "") // neu ko co chon loai xe thi tat ca loai xe           
        ...
        private void CapNhapCuocGoiTuTongDai(ref List<CuocGoi> listCuocGoiHienTai, string linesDuocCapPhep,
            DateTime thoiDiemNhanDulieuTruoc, ref bool hasCuocGoiThayDoi, ref DateTime DateMax, ref bool hasCuocGoiMoi)
        {
            hasCuocGoiThayDoi = false;
            // Lấy ds cuộc gọi có thay đổi thông tin của tổng đài
            List<CuocGoi> listCuocGoi = CuocGoi.DIENTHOAI_LayDSCuocGoiThayDoiByTongDai_V3(linesDuocCapPhep, thoiDiemNhanDulieuTruoc);
            if (listCuocGoi != null && listCuocGoi.Count > 0)
            {
                foreach (CuocGoi objCG in listCuocGoi)
                {
                    ...
                    {
                        TimVaCapNhatCuocGoi(ref listCuocGoiHienTai, objCG, false, false);
                    }
                }
            }
        }
        ...
        /// <param name="linesDuocCapPhep"></param>
        /// <param name="hasCuocGoiMoi"></param>
        private void CapNhatCuocGoiKetThucTuTongDai(ref List<CuocGoi> listCuocGoiHienTai, string linesDuocCapPhep, ref bool hasCapNhat)
        {
            hasCapNhat = false;
            // lấy ds các ID cuộc gọi hiện có
            string listIDCuocGoi = "";
            ...
            if (listCuocGoiHienTai != null && listCuocGoiHienTai.Count > 0)
            {
                ...
                for (int i = 0; i < listCuocGoiHienTai.Count; i++)
                {
                    if (listCuocGoiHienTai[i].IDCuocGoi == cuocGoi.IDCuocGoi)
                    {
                        ...
                        if (index >= 0)
                        {
                        var cuocgoiCurret = listCuocGoiHienTai[index];
                        if (STaxi)
                        {
                            cuocgoiCurret.KieuCuocGoi = cuocGoi.KieuCuocGoi;
                            cuocgoiCurret.LoaiXe = cuocGoi.LoaiXe;
                            cuocgoiCurret.SoLuong = cuocGoi.SoLuong;
                            cuocgoiCurret.Vung = cuocGoi.Vung;
                            cuocgoiCurret.FT_KM = cuocGoi.FT_KM;
                            cuocgoiCurret.DanhSachXeDeCu = cuocGoi.DanhSachXeDeCu;
                            cuocgoiCurret.DanhSachXeDeCu_TD = cuocGoi.DanhSachXeDeCu_TD;
                        }
                        else
                        {
                            if (!IsCapNhatCuaDienThoai) // cap nhat theo du lieu tong dai gui sang
                            {
                                cuocgoiCurret.TrangThaiCuocGoi = cuocGoi.TrangThaiCuocGoi;
                                cuocgoiCurret.LenhTongDai = cuocGoi.LenhTongDai;
                                cuocgoiCurret.GhiChuTongDai = cuocGoi.GhiChuTongDai;
                                cuocgoiCurret.MaNhanVienTongDai = cuocGoi.MaNhanVienTongDai;
                                cuocgoiCurret.LenhDienThoai = cuocGoi.LenhDienThoai;
                                cuocgoiCurret.GhiChuDienThoai = cuocGoi.GhiChuDienThoai;
                                cuocgoiCurret.MaNhanVienDienThoai = cuocGoi.MaNhanVienDienThoai;
                                cuocgoiCurret.XeNhan = cuocGoi.XeNhan;
                                cuocgoiCurret.XeNhan_TD = cuocGoi.XeNhan_TD;
                                cuocgoiCurret.ThoiGianDieuXe = cuocGoi.ThoiGianDieuXe;
                                cuocgoiCurret.FileVoicePath = cuocGoi.FileVoicePath;
                                cuocgoiCurret.VungGPSID = cuocGoi.VungGPSID;
                                cuocgoiCurret.GPS_KinhDo = cuocGoi.GPS_KinhDo;
                                cuocgoiCurret.GPS_ViDo = cuocGoi.GPS_ViDo;
                                cuocgoiCurret.DanhSachXeDeCu = cuocGoi.DanhSachXeDeCu;
                                cuocgoiCurret.ThoiDiemCapNhatXeDeCu = cuocGoi.ThoiDiemCapNhatXeDeCu;
                                cuocgoiCurret.TrangThaiLenh = cuocGoi.TrangThaiLenh;
                                cuocgoiCurret.MOIKHACH_LenhMoiKhach = cuocGoi.MOIKHACH_LenhMoiKhach;
                                cuocgoiCurret.MOIKHACH_NhanVien = cuocGoi.MOIKHACH_NhanVien;
                                cuocgoiCurret.XeDenDiem = cuocGoi.XeDenDiem;
                                cuocgoiCurret.XeDenDiemDonKhach = cuocGoi.XeDenDiemDonKhach;
                                cuocgoiCurret.Vung = cuocGoi.Vung;
                                cuocgoiCurret.DiaChiDonKhach = cuocGoi.DiaChiDonKhach;
                                cuocgoiCurret.PhoneNumber = cuocGoi.PhoneNumber;
                                cuocgoiCurret.DiaChiTraKhach = cuocGoi.DiaChiTraKhach;
                                cuocgoiCurret.SoLuong = cuocGoi.SoLuong;
                                cuocgoiCurret.KieuCuocGoi = cuocGoi.KieuCuocGoi;
                            }
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    cuocgoiCurret.SoLuong = cuocGoi.SoLuong;
                                    cuocgoiCurret.Vung = cuocGoi.Vung;
                                    cuocgoiCurret.XeDon = cuocGoi.XeDon;
                                    cuocgoiCurret.XeNhan = cuocGoi.XeNhan;
                                    cuocgoiCurret.XeDenDiem = cuocGoi.XeDenDiem;
                                }
                            }
        ...
        private void TimVaCapNhatCuocGoi_V2(ref List<CuocGoi> listCuocGoiHienTai, CuocGoi cuocGoi, bool IsCapNhatCuaDienThoai, ref bool HasThemMoi, bool STaxi = false)
        {
            if (listCuocGoiHienTai != null && listCuocGoiHienTai.Count > 0)
            {
                int index = -1;
        ...
        private void HienThiFormGoiDienThoai(string PhoneNumber, string DiaChi, bool isPBXIP)
        {
            ...
            if (g_COMPort.Length > 0 || isPBXIP)
            {
                frmCalling.Show();
                frmCalling.Invoke(
                    (MethodInvoker) delegate()
                    {
                        frmCalling.lblSoGoi.Text = String.Format("Đang gọi :{2} - {0} - {1}", PhoneNumber, DiaChi, g_lineIPPBX);
                    }
                    );
                frmCalling.Refresh();
        ...
        //        new MessageBox.MessageBox().Show("Không kết nối được với Service đồng bộ xe", "Thông Báo",
        //            MessageBox.MessageBoxButtons.OK, MessageBox.MessageBoxIcon.Warning);
        //    }
        //}

        void bwSync_UpdateCuocGoi_DoWork(object sender, DoWorkEventArgs e)
        {
                   ...
                   // updateSuccess = false;
                    ...
                    if (QueueCuocGoi.TryDequeue(out cuocGoi))
                    {
                        try
                        {
                            ...
                            getXeDeCuByToaDo(KinhDo, ViDo, cuocGoi.LoaiXe, cuocGoi.ThoiDiemGoi_FT, cuocGoi.IDCuocGoi, cuocGoi.PhoneNumber, cuocGoi.DiaChiDonKhach, ref dsXeDeCu, ref dsXeDeCu_TD);
                            ...
                            cuocGoi.FT_KM = ProcessFastTaxi.TinhQuangDuong(cuocGoi.FT_Cust_Lat, cuocGoi.FT_Cust_Lng, (float)cuocGoi.GPS_ViDo, (float)cuocGoi.GPS_KinhDo);
        ...
        private void grid_PhanAnhKhieuNai_KeyDown(object sender, KeyEventArgs e)
        {

        }

        private void grid_PhanAnhKhieuNai_DoubleClick(object sender, EventArgs e)
        {

        }

        private void uiCommandManager1_CommandClick(object sender, CommandEventArgs e)
        {
        ...
        {
            LogError.WriteLogInfo("rbDefault_CheckedChanged");
            if (rbDefault.Checked && !G_IsChangingLine)
            {
                SetControl_LoaiXe_MacDinh();
                ReloadAllTaxiOperation_Line(g_LinesDuocCapPhep);
                //LogError.WriteLogInfo("rbDefault_CheckedChanged");
            }
        }
        ...
        {
            LogError.WriteLogInfo("rbAll_CheckedChanged");
            if (rbAll.Checked && !G_IsChangingLine)
            {
                SetLine_Custom();
                ReloadAllTaxiOperation_Line(g_LinesDuocCapPhep_Custom);
                //LogError.WriteLogInfo("rbAll_CheckedChanged");
            }
        }
        ...
        {
            LogError.WriteLogInfo("rbSupport_CheckedChanged");
            if (rbSupport.Checked && !G_IsChangingLine)
            {
                g_LinesDuocCapPhep_Support = StringTools.RemoveSpace(cbLineCapPhep.GetValue().ToString());
                g_LinesDuocCapPhep_Custom = g_LinesDuocCapPhep;
                ReloadAllTaxiOperation_Line(g_LinesDuocCapPhep_Support);
                //LogError.WriteLogInfo("rbSupport_CheckedChanged");
            }
        }
        
        private void SetControl_LoaiXe_MacDinh()
        {
            LogError.WriteLogInfo("SetControl_LoaiXe_MacDinh");
            G_IsChangingLine = true;
            cbLineCapPhep.SetValue(g_LinesDuocCapPhep);
            
            g_LinesDuocCapPhep_Custom = g_LinesDuocCapPhep_Support = g_LinesDuocCapPhep;
            G_IsChangingLine = false;
        }

        private void SetLine_Custom()
        {
            LogError.WriteLogInfo("SetLine_Custom");
            g_LinesDuocCapPhep_Custom = g_LinesDuocCapPhep_Support = StringTools.RemoveSpace(cbLineCapPhep.GetValue().ToString());
        }

        private void ReloadAllTaxiOperation_Line(string Line)
        {
            LogError.WriteLogInfo("ReloadAllTaxiOperation_Line");
            LoadAllCuocGoiHienTai(Line);
            HienThiTrenLuoi(true, true);
        }

        private void cbLineCapPhep_EditValueChanged(object sender, EventArgs e)
        {
            LogError.WriteLogInfo("cbLineCapPhep_EditValueChanged");
            G_IsChangingLine = true;
